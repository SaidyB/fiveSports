...
const Home = () => {
    ...
    const filteredProducts = products.filter(product => {
      if (!product.bookings || product.bookings.length === 0) {
        ...
        return product.bookings.every(booking => {
          ...
          return (finalDate.isBefore(bookingStart) || inicioDate.isAfter(bookingEnd));
        });
      }
    });
  ...
  const buscarProductos =()=>{
    if (selectedCategory !== 'Categoría' && productsAvailable.length > 0) {
      ...
      const filteredProductsByCategory = productsAvailable.filter(product =>
      ...
      <div className="container_reservation">
        ...
        <div className="container_calendar">
          <div className="textCategory">
            ...
            {openMenuCategory && (
              <div className="menu">
                <h4 onClick={() => handleCategoryClick("Gimnasio")}>Gimnasio</h4>
                <h4 onClick={() => handleCategoryClick("Deportes")}>Deportes</h4>
          ...
          <div className="textBuscar">
            <Button onClick={buscarProductos} className="btnBuscar bg-indigo-400 border-indigo-400">Buscar</Button>
          </div>
        </div>
      </div>
      {mostrarFiltrados&&(
